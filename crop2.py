...
crop = le.fit_transform(list(excel["label"]))                              # Mapping the values in weather into numerical form.

                                                    
NITROGEN = list(excel["N"])                                        # Making the whole row consisting of nitrogen values to come into nitrogen. 
PHOSPHORUS = list(excel["P"])                                    # Making the whole row consisting of phosphorus values to come into phosphorus. 
POTASSIUM = list(excel["K"])                                      # Making the whole row consisting of potassium values to come into potassium. 
...
layout = [[sg.Text('                      Crop Recommendation Assistant', font=("Helvetica", 30), text_color = 'yellow')],                                                    # Defining the layout of the Graphical User Interface. It consist of some text, Buttons, and blanks to take Input.                                                        
         ...
         [sg.Text('Enter                           :', font=("Helvetica", 20)), sg.Input(font=("Helvetica", 20),size = (20,1))]]
...
while True: 
	...
	if event == sg.WINDOW_CLOSED or event == 'Quit':                                                                                            # If the user will press the quit button then the program will end up.
		break
	print(values[0])
	nitrogen_content =         float(values[0])                                                                                                        # Taking input from the user about nitrogen content in the soil.
	phosphorus_content =      float( values[1])                                                                                                        # Taking input from the user about phosphorus content in the soil.
	potassium_content =        float(values[2])                                                                                                        # Taking input from the user about potassium content in the soil.
	temperature_content =      float(values[3])                                                                                                        # Taking input from the user about the surrounding temperature.
	humidity_content =         float(values[4])                                                                                                        # Taking input from the user about the surrounding humidity. 
	ph_content =               float(values[5])                                                                                                        # Taking input from the user about the ph level of the soil.
	rainfall =                 float(values[6])                                                                                                        # Taking input from the user about the rainfall.
